...
struct bneck bneck_list[MAX_NECK_COUNT];
...
int find_MaxNeighborResource_node(struct s2v_node * s2v_n, struct s2v_link * s2v_l, double cpu, int index) {
  int i, j, snode = -1;
  double max = -1;
  double diff, sum;
  for(i = 0; i < sub.nodes; i ++) {
...
int map_node_greedy(struct s2v_node * s2v_n, struct s2v_link * s2v_l, struct req2sub * v2s, int index) {
  ...
  for (i = 0; i < req[index].nodes; i ++) {
    ...
    t = find_MaxNeighborResource_node(s2v_n, s2v_l, req[index].cpu[i], index);
    if (t == -1) {
...
int check_flow(struct s2v_node * s2v_n, struct s2v_link * s2v_l, struct req2sub * v2s, int start, int end, int option, int time) {
  ...
  *vlink = -1;
  ...
  if (strstr(str, "INFEASIBLE") != NULL && strstr(str, "PHASE 1") != NULL) {
    memset(bneck_list, 0, sizeof(struct bneck) * MAX_NECK_COUNT);
    ...
    for (i = 0; i < sub.links; i ++) {
      ...
      if (flow > cap) {
        ...
        for (l = 0; l < s2v_ltmp[2 * index].count; l ++) {
          //printf("bw %d %.2f\n", i, s2v_ltmp[2 * index].bw[i]);
          if (s2v_ltmp[2 * index].bw[l] > maxbw) {
            maxid = l;
            maxbw = s2v_ltmp[2*index].bw[l];
          }
        }
        for (l = 0; l < s2v_ltmp[2 * index+1].count; l ++) {
          if (s2v_ltmp[2 * index+1].bw[l] > maxbw) {
            maxid = l;
            maxbw = s2v_ltmp[2*index+1].bw[l];
          }
        }
        ...
        int tmp = 0;
        for (l = start; l <= end; l ++) {
          if (req[l].split == LINK_SPLITTABLE && (v2s[l].map == STATE_MAP_NODE || (v2s[l].map == STATE_MAP_LINK && option == ROUTE_MIGRATION))) {
            if (tmp + req[l].links > maxid) {
              tmp = maxid - tmp;
              break;
            } else {
              tmp += req[l].links;
        ...
        bneck_list[rvalue].slink = index - 1;
        bneck_list[rvalue].vlink = tmp;
        bneck_list[rvalue].req = l;
        rvalue ++;
      }
    }
  } else {
    ...
    if (bottleneck_req == -1) {  //success in get flow
      ...
      if (bottleneck_req == -2) {
        ...
        if (trycount > TIMES_TRY) {
          ...
          int neck;
          for (neck = 0; neck < bottleneck_req; neck ++) {

            if (rand()/(double)RAND_MAX < 0.5) {
              vnode = req[neck_list[neck].req].link[neck_list[neck].vlink].from;
            } else {
              vnode = req[neck_list[neck].req].link[neck_list[neck].vlink].to;
            }     
            int snode = v2stmp[neck_list[neck].req].snode[vnode];
            for (i = 0; i < s2v_ntmp[snode].req_count; i ++) {
              if (s2v_ntmp[snode].req[i] == neck_list[neck].req) {
                break;
              }
            }
            ...
            reqid = i;
            index = neck_list[neck].req;

            t = find_MaxNeighborResource_node(s2v_ntmp, s2v_ltmp, s2v_ntmp[snode].cpu[reqid], index);
            if (t == -1) {
              printf("req %d unsatisfied\n", index);
              trycount = TIMES_TRY +1;
            } else {
...
int main(int argc, char ** argv) {
  ...
  sprintf(filename, "sub.txt");
    ...
    reqcount = 0;
    for (i = 0; i < n; i ++) {
      if (v2s[i].map == STATE_DONE || v2s[i].map == STATE_MAP_LINK) {
        bwsum = 0;
        reqcost = 0;
        for (j = 0; j < req[i].links; j ++) {
          bwsum += req[i].link[j].bw;
    ...
    fprintf(fp, "%d %lf", count, opcost/(double)count);
